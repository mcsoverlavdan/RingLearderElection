...
func (p *DataPasser) log() {
	...
	for item := range p.queue {
		...
		if err!=nil{
			routingTable["firsthop"]=Baddr
			routingTable["nexthop"]=Caddr
			c,err=ConnectToAddress("firsthop")
		}
		...
		if m.Action =="election" {
			...
			if err = controllerAddr.Connection.WriteMessage(1, []byte("initiating value D 120")); err != nil {
				fmt.Println(err)
			}
