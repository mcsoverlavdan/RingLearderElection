...
func ConnectToAddress(adressval string)(*websocket.Conn,error){
	...
	if err != nil {
		...
		panic("dial to address failed trying address")
...
func handlepanic() {
	...
	if a := recover(); a != nil {
		routingTable["firsthop"]=Aaddr
		routingTable["nexthop"]=Baddr
...
func (p *DataPasser) log() {
	...
	for item := range p.queue {
		...
		if m.Action =="election" {
			...
			if err = controllerAddr.Connection.WriteMessage(1, []byte("initiating value C 110")); err != nil {
				fmt.Println(err)
			}
