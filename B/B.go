...
func (p *DataPasser) log() {
	...
	for item := range p.queue {
		...
		if err!=nil{
			routingTable["firsthop"]=Daddr
			routingTable["nexthop"]=Aaddr
			c,err=ConnectToAddress("firsthop")
		}
		...
		if m.Action =="election" {
			...
			if err = controllerAddr.Connection.WriteMessage(1, []byte("initiating value B 100")); err != nil {
				fmt.Println(err)
			}
